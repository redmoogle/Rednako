...
class DefaultPlayer(BasePlayer):
    ...
    def __init__(self, guild_id, node):
        ...
        self.pitch = 1
        self.rotation = 0
        self.vibrato = {"frequency": 2, "depth": 0.5}
        self.tremolo = {"frequency": 2, "depth": 0.5}
        self.shuffle = False
        self.repeat = False
    ...
    async def set_speed(self, speed: int):
        """
        Sets the player's timescale

        Parameters
        ----------
        vol: :class:`int`
            The new timescale level.
        """
        self.speed = speed
        await self.node._send(op='filters', guildId=self.guild_id, timescale={"speed": self.speed})
        
    async def set_pitch(self, pitch: int):
        """
        ...
        """
        self.pitch = pitch
        await self.node._send(op='filters', guildId=self.guild_id, timescale={"pitch": self.pitch})

    async def set_rotation(self, rotation: float):
        """
        Sets the player's rotation in Hz
        ...
        await self.node._send(op='filters', guildId=self.guild_id, rotation={"rotationHz": self.rotation})

    async def set_vibrato(self, frequency: int, depth: int):
        """
        Sets the player's vibrato
        ...
        depth: :class:`int`
            The new depth.
        """
        self.vibrato["frequency"] = max(min(frequency, 14), 0.001)
        self.vibrato["depth"] = max(min(depth, 1), 0.001)
        await self.node._send(op='filters', guildId=self.guild_id, timescale={"vibrato": self.vibrato})

    async def set_tremolo(self, frequency: int, depth: int):
        """
        Sets the player's tremolo
        ...
        depth: :class:`int`
            The new depth.
        """
        self.tremolo["frequency"] = max(min(frequency, 14), 0.001)
        self.tremolo["depth"] = max(min(depth, 1), 0.001)
        await self.node._send(op='filters', guildId=self.guild_id, timescale={"tremolo": self.tremolo})
